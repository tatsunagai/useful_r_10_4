...
int initR(int argc, char **argv)
{
	...
	/* set RFLOW_MAX_MEMORY env*/
    {
    	char maxMemoryChar[100];
    	unsigned long long maxMemory;
		MEMORYSTATUSEX ms;
		ms.dwLength = sizeof(MEMORYSTATUSEX);
		GlobalMemoryStatusEx(&ms);
#ifdef WIN64
		maxMemory = ms.ullTotalPhys;
#else
		maxMemory = min(ms.ullTotalVirtual - 512*1048576, ms.ullTotalPhys);
#endif
		maxMemory = max(32*1048576, maxMemory);
    	sprintf(maxMemoryChar, "RFLOW_MAX_MEMORY=%llu", maxMemory);
    	putenv(maxMemoryChar);
    }
